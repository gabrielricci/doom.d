...
(defun chatgpt--fix-newlines (text)
  "Replace one or more newline characters with two newline characters in TEXT. Function no longer used!"
...
(defun chatgpt--parse-response (status callback)
  ...
  (goto-char 0)
  (re-search-forward "^$")
  (funcall callback (chatgpt--fix-newlines (chatgpt--extract-text-from-query (json-read)))))
...
https://beta.openai.com/docs/introduction/key-concepts"
  ...
  (let* ((url "https://api.openai.com/v1/completions")
         ...
         (url-request-data (json-encode json-object)))
    (url-retrieve
     url
